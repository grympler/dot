...
if [[ $(zsh --version | awk '{printf("%.1f", $2)}') > 5.2 ]]; then
    ...
    () {
      ...
      typeset -ga POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
          custom_rprompt status command_execution_time background_jobs context)
      ...
      function custom_rprompt() {
        if [[ ! -z "$CONDA_PREFIX" ]]; then
            echo -n "($(basename $CONDA_PREFIX))"
        fi
      }  # redefine this to show stuff in custom_rprompt segment
